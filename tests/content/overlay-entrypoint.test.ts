...
beforeEach(() => {
  ...
  Object.defineProperty(globalThis, "chrome", {
    ...
    value: {
      runtime: {
        onMessage: { addListener: vi.fn((candidate: (message: unknown) => void) => { listener = candidate; }) },
        sendMessage,
      },
    },
  });
...
describe("overlay content entrypoint", () => {
  it("starts once, announces readiness, and ignores malformed messages", () => {
    ...
    expect(sendMessage).toHaveBeenCalledWith({ kind: "content-ready", url: window.location.href });
