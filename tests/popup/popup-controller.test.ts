...
import { PopupController, type PopupImporter, type PopupRuntimeAdapter, type PopupView } from "../../src/popup/popup-controller";
import type { ImportedReference, OverlaySnapshot, PopupRequest, SettingsPatch, TabState } from "../../src/shared/contracts";
...
function reference(): ImportedReference {
  const parsed = parseImportedReference({
    metadata: { id: "123e4567-e89b-42d3-a456-426614174000", name: "reference.png", mimeType: "image/png", width: 20, height: 10, encodedBytes: 26, importedAt: 1 },
...
function snapshot(input: Readonly<{ reference?: ImportedReference | null; settings?: Partial<OverlaySnapshot["settings"]>; revision?: number }> = {}): OverlaySnapshot {
  const parsed = new URL(url);
  const imported = input.reference === undefined ? reference() : input.reference;
  return {
    ...
    settings: {
      ...
      sizing: input.settings?.sizing ?? { kind: "fit-width", lastScalePercent: 100 },
      interactionMode: input.settings?.interactionMode ?? "click-through",
    },
  };
}

function tab(input: Readonly<{ enabled?: boolean; snapshot?: OverlaySnapshot }> = {}): TabState {
  const current = input.snapshot ?? snapshot();
  return { tabId: 1, url, origin: current.origin, enabled: input.enabled ?? true, snapshot: current, diagnostic: null };
}

function applyPatch(current: OverlaySnapshot, patch: SettingsPatch): OverlaySnapshot {
  const settings = patch.kind === "visibility" ? { ...current.settings, visible: patch.visible }
    : patch.kind === "opacity" ? { ...current.settings, opacity: patch.opacity }
    : patch.kind === "inversion" ? { ...current.settings, inverted: patch.inverted }
    : patch.kind === "sizing" ? { ...current.settings, sizing: patch.sizing }
    : patch.kind === "interaction-mode" ? { ...current.settings, interactionMode: patch.interactionMode }
    : { ...current.settings, placement: patch.placement };
  return { ...current, revision: current.revision + 1, settings };
}

type Harness = Readonly<{
  adapter: PopupRuntimeAdapter & { requests: PopupRequest[]; resolveNext: ((value: unknown) => void) | undefined };
  view: PopupView & { states: ReturnType<typeof vi.fn>; focus: ReturnType<typeof vi.fn> };
  controller: PopupController;
}>;

function createHarness(input: Readonly<{ activeUrl?: string | null; enabled?: boolean; importer?: PopupImporter; pauseSettings?: boolean }> = {}): Harness {
  ...
  const adapter: Harness["adapter"] = {
    ...
    async getActiveUrl() { return input.activeUrl === undefined ? url : input.activeUrl; },
    async requestOrigin() { return true; },
    send(request) {
      requests.push(request);
      if (input.pauseSettings && request.kind === "update-settings" && held === undefined) {
        return new Promise((resolve) => { held = resolve; adapter.resolveNext = resolve; });
      }
      if (request.kind === "get-tab-state") return Promise.resolve({ requestId: request.requestId, ok: true, value: current });
      if (request.kind === "register-site") {
        current = { ...current, enabled: true };
        return Promise.resolve({ requestId: request.requestId, ok: true, value: current.snapshot });
      }
      if (request.kind === "update-settings") {
        current = { ...current, snapshot: applyPatch(current.snapshot, request.patch) };
        return Promise.resolve({ requestId: request.requestId, ok: true, value: current.snapshot });
      }
      if (request.kind === "replace-reference") {
        current = { ...current, snapshot: { ...current.snapshot, reference: request.reference.metadata, revision: current.snapshot.revision + 1 } };
        return Promise.resolve({ requestId: request.requestId, ok: true, value: current.snapshot });
      }
      current = { ...current, enabled: false, snapshot: snapshot({ reference: null }) };
      return Promise.resolve({ requestId: request.requestId, ok: true, value: undefined });
    },
  };
  const view: Harness["view"] = { states: vi.fn(), focus: vi.fn(), render: vi.fn(), restoreFocus: vi.fn() };
  view.render = view.states;
  view.restoreFocus = view.focus;
  const importer = input.importer ?? vi.fn(async () => ({ ok: true as const, value: reference() }));
  return { adapter, view, controller: new PopupController({ adapter, view, importer }) };
}
...
describe("popup controller", () => {
  it("renders unsupported, access-required, empty, and reference states", async () => {
    ...
    empty.adapter.send = async (request) => request.kind === "get-tab-state"
      ? { requestId: request.requestId, ok: true, value: tab({ snapshot: snapshot({ reference: null }) }) }
  ...
  it("retains stable import errors, then imports a successful replacement", async () => {
    const importer = vi.fn()
      .mockResolvedValueOnce({ ok: false, error: { code: "image-decode-failed", message: "Pixel Pincher could not decode that image." } })
      .mockResolvedValueOnce({ ok: true, value: reference() });
    const harness = createHarness({ importer });
    await harness.controller.start();
    await harness.controller.importFile(new File(["x"], "bad.png", { type: "image/png" }));
    expect(harness.controller.state.kind).toBe("error");
    await harness.controller.importFile(new File(["x"], "good.png", { type: "image/png" }));
    expect(harness.adapter.requests.some((request) => request.kind === "replace-reference")).toBe(true);
  ...
  it("coalesces opacity updates to the latest value and allows transparency endpoints", async () => {
    ...
    harness.adapter.resolveNext?.({ requestId: first.requestId, ok: true, value: snapshot({ settings: { opacity: 0 } }) });
    await settled();
    const updates = harness.adapter.requests.filter((request) => request.kind === "update-settings");
    expect(updates).toHaveLength(2);
    expect(updates[1]).toMatchObject({ patch: { kind: "opacity", opacity: 1 } });
  });
  ...
  it("preserves manual scale across fit width, clamps sizing, resets, and updates inversion", async () => {
    ...
    harness.controller.updateSettings({ kind: "inversion", inverted: true }, "inverted");
    await settled();
    const patches = harness.adapter.requests.filter((request): request is Extract<PopupRequest, { kind: "update-settings" }> => request.kind === "update-settings").map((request) => request.patch);
    expect(patches).toContainEqual({ kind: "sizing", sizing: { kind: "fit-width", lastScalePercent: 150 } });
    expect(patches).toContainEqual({ kind: "sizing", sizing: { kind: "scale", percent: 150 } });
    expect(patches).toContainEqual({ kind: "sizing", sizing: { kind: "scale", percent: 400 } });
    expect(patches).toContainEqual({ kind: "sizing", sizing: { kind: "scale", percent: 100 } });
  ...
  it("parses integer controls, supports keyboard steps, and requires clear confirmation", async () => {
    ...
    await settled();
    expect(harness.adapter.requests.filter((request) => request.kind === "update-settings")).toHaveLength(3);
    harness.controller.clearSite();
    expect(harness.adapter.requests.some((request) => request.kind === "clear-site")).toBe(false);
