...
describe("boundary parsers", () => {
  it("accepts only canonical supported URLs, origins, and page keys", () => {
    ...
    expect(errorCode(parseSupportedUrl("chrome://extensions/"))).toBe(
      "unsupported-url",
    );
  ...
  it("enforces canonical UUID-v4 reference IDs", () => {
    ...
    expect(parseReferenceId("123e4567-e89b-12d3-a456-426614174000").ok).toBe(
      false,
    );
  });
  ...
  it("validates settings bounds, variants, and exact fields", () => {
    ...
    expect(
      parseSettingsPatch({ kind: "inversion", inverted: true, extra: true }).ok,
    ).toBe(false);
  });
  ...
  it("rejects malformed, oversized, or mismatched image references", () => {
    ...
    expect(
      parseReferenceMetadata({ ...metadata, width: 40_000_001, height: 1 }).ok,
    ).toBe(false);
    expect(
      parseReferenceMetadata({
        ...metadata,
    ...
    expect(
      parseImportedReference({
        ...importedReference,
        metadata: { ...metadata, encodedBytes: 2 },
      }).ok,
    ).toBe(false);
    expect(
      parseImageRecordV1({ schemaVersion: 1, referenceId: id, dataUrl }).ok,
    ).toBe(true);
    expect(
      parseImageRecordV1({
        schemaVersion: 1,
  ...
  it("validates multi-megabyte base64 payloads without recursive regex matching", () => {
    ...
    expect(
      parseImportedReference({
        metadata: {
          ...metadata,
          encodedBytes: new TextEncoder().encode(largeDataUrl).byteLength,
        },
        dataUrl: largeDataUrl,
      }),
    ).toEqual({
      ok: true,
      value: expect.objectContaining({ dataUrl: largeDataUrl }),
    });
  });
  ...
  it("parses each popup request and rejects accessors, inherited fields, and extras", () => {
    ...
    expect(
      parsePopupRequest({ kind: "get-tab-state", requestId: "a", extra: true })
        .ok,
    ).toBe(false);
  ...
  it("makes hydration constructive and requires complete metadata equality", () => {
    expect(parseHydration({ snapshot, reference: importedReference }).ok).toBe(
      true,
    );
    expect(
      parseHydration({
    ...
    expect(
      parseHydration({
        ...
        reference: {
          ...importedReference,
          metadata: { ...metadata, id: "123e4567-e89b-42d3-b456-426614174000" },
        },
      }).ok,
    ).toBe(false);
    expect(
      parseHydration({
        ...
        reference: {
          ...importedReference,
          metadata: { ...metadata, name: "other.png" },
        },
      }).ok,
    ).toBe(false);
  });
  ...
  it("rejects malformed, future, and inconsistent storage records", () => {
    ...
    expect(
      parseOriginRecordV1({
        ...originRecord,
        settings: { ...originRecord.settings, placement: { x: 0, y: 0 } },
      }).ok,
    ).toBe(false);
    expect(
  ...
  it("parses every strict content message variant and safe revisions", () => {
    expect(
      parseContentRequest({
        ...
        hydration: { snapshot, reference: importedReference },
      }).ok,
    ).toBe(true);
    expect(parseContentRequest({ kind: "apply-settings", snapshot }).ok).toBe(
      true,
    );
    expect(parseContentRequest({ kind: "clear-overlay", revision: 1 }).ok).toBe(
      true,
    );
    expect(
      parseContentRequest({
  ...
  it("parses only reference-aware render diagnostics in tab state", () => {
    const diagnostic = {
      referenceId: id,
      error: publicError("image-render-failed"),
    };
    expect(
      parseTabState({
        tabId: 3,
    ...
    expect(
      parseTabState({
        ...
        origin: "https://example.com",
        enabled: true,
        snapshot: { ...snapshot, panelPosition: { x: 24, y: 24 } },
        diagnostic,
      }).ok,
    ).toBe(true);
  });
  ...
  it("parses canonical popup responses and every request variant", () => {
    ...
    const numberParser = (value: unknown): Result<number, PublicError> =>
      typeof value === "number"
        ? { ok: true, value }
      ...
      parsePopupResponse(
        {
          requestId: "response-1",
          ok: false,
          error: { code: "storage-failed", message: "raw Chrome error" },
        },
        numberParser,
      ).ok,
    ).toBe(false);
    ...
    expect(
      parsePopupRequest({ kind: "get-tab-state", requestId: "a" }).ok,
    ).toBe(true);
    expect(
      parsePopupRequest({
        kind: "register-site",
  ...
  it("covers every settings patch variant and its malformed counterpart", () => {
    const patches = [
      {
        valid: { kind: "visibility", visible: true },
        invalid: { kind: "visibility", visible: "true" },
      },
      {
        valid: { kind: "opacity", opacity: 0.25 },
        invalid: { kind: "opacity", opacity: -0.1 },
      },
      {
        valid: { kind: "inversion", inverted: true },
        invalid: { kind: "inversion", inverted: 1 },
      },
      {
      ...
      },
      {
        ...
        invalid: { kind: "interaction-mode", interactionMode: "pass-through" },
      },
      {
        valid: { kind: "placement", placement: { x: 1, y: -1 } },
        invalid: { kind: "placement", placement: { x: 1.5, y: -1 } },
      },
    ];
    for (const patch of patches) {
      expect(parseSettingsPatch(patch.valid).ok).toBe(true);
      expect(parseSettingsPatch(patch.invalid).ok).toBe(false);
      expect(parseSettingsPatch({ ...patch.valid, extra: true }).ok).toBe(
        false,
      );
    }
  ...
  it("covers popup and content variants, malformed fields, extras, and unknown discriminants", () => {
    ...
    expect(parsePopupRequest({ kind: "unknown", requestId: "a" }).ok).toBe(
      false,
    );
    expect(
      parsePopupRequest({
    ...
    expect(
      parseContentRequest({ kind: "clear-overlay", revision: -1 }).ok,
    ).toBe(false);
    ...
    expect(
      parseContentEvent({ kind: "content-ready", url: "ftp://example.com/" })
        .ok,
    ).toBe(false);
  });
  ...
  it("accepts every public error code and rejects unknown or extra errors", () => {
    for (const [code, message] of Object.entries({
      ...
      "image-too-large":
        "The image is too large. Choose an image up to 10 MiB.",
      ...
      "image-render-failed":
        "Pixel Pincher could not render the reference image.",
    })) {
      expect(parsePublicError({ code, message }).ok).toBe(true);
    }
    expect(parsePublicError({ code: "unknown", message: "unknown" }).ok).toBe(
      false,
    );
    expect(
      parsePublicError({
