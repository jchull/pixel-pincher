...
function parseHydrationWithPanelPosition(
  value: unknown,
): Result<Hydration, PublicError> {
  if (
  ...
  if (
    !isOwnDataRecord(value.snapshot) ||
    !Object.hasOwn(value.snapshot, "panelPosition")
  ) {
  ...
  return {
    ...
    value: {
      snapshot: {
        ...snapshot.value,
        reference: parsed.value.reference.metadata,
      },
...
export function parseContentPanelResponse(
  ...
  if (!hasExactKeys(value, ["requestId", "ok", "value"]))
    return failure("invalid-request");
