...
export function parseReferenceId(
  value: unknown,
): Result<ReferenceId, PublicError> {
  const raw = readString(value);
  if (raw === undefined || !UUID_V4.test(raw))
    return failure("invalid-request");
  return { ok: true, value: brandReferenceId(raw) };
}

function parsePlacementValue(
  value: unknown,
  code: ParseErrorCode,
): Result<Placement, PublicError> {
...
function parseSizingValue(
  ...
  if (value.kind === "fit-width") {
    ...
    if (
      !hasExactKeys(value, ["kind", "lastScalePercent"]) ||
  ...
  if (value.kind === "scale") {
    ...
    if (
      !hasExactKeys(value, ["kind", "percent"]) ||
      percent === undefined ||
...
function parseSettingsValue(
  value: unknown,
  code: ParseErrorCode,
): Result<OverlaySettings, PublicError> {
  if (
...
function parseMetadataValue(
  value: unknown,
  code: ParseErrorCode,
): Result<ReferenceMetadata, PublicError> {
  if (
    !isOwnDataRecord(value) ||
    !hasExactKeys(value, [
      "id",
...
function isBase64Payload(value: string): boolean {
  ...
  if (
    paddingLength > 2 ||
    (paddingLength !== 0 && contentEnd + paddingLength !== value.length)
  )
    return false;
...
function parseDataUrl(
  ...
  if (
    dataUrl === undefined ||
    utf8ByteLength(dataUrl) > MAX_IMAGE_ENCODED_BYTES
  )
    return failure(code);
  ...
  if (
    separatorIndex === -1 ||
    dataUrl.indexOf(separator, separatorIndex + separator.length) !== -1
  )
    return failure(code);
  ...
  if (
    ...
    !isBase64Payload(payload)
  )
    return failure(code);
  const encodedBytes = utf8ByteLength(dataUrl);
  if (expectedBytes !== undefined && encodedBytes !== expectedBytes)
    return failure(code);
  return { ok: true, value: dataUrl };
}

function parseImportedReferenceValue(
  value: unknown,
  code: ParseErrorCode,
): Result<ImportedReference, PublicError> {
  if (!isOwnDataRecord(value) || !hasExactKeys(value, ["metadata", "dataUrl"]))
    return failure(code);
  ...
  const dataUrl = parseDataUrl(
    ...
    metadata.value.encodedBytes,
  );
  if (!dataUrl.ok) return dataUrl;
  return {
    ok: true,
    value: { metadata: metadata.value, dataUrl: dataUrl.value },
  };
}

export function parseImportedReference(
  value: unknown,
): Result<ImportedReference, PublicError> {
  return parseImportedReferenceValue(value, "invalid-request");
}

function parseSnapshotValue(
  value: unknown,
  code: ParseErrorCode,
...
export function parseOverlaySnapshot(
  value: unknown,
): Result<OverlaySnapshot, PublicError> {
...
export function parseHydration(value: unknown): Result<Hydration, PublicError> {
  if (
    !isOwnDataRecord(value) ||
    !hasExactKeys(value, ["snapshot", "reference"])
  )
    return failure("invalid-request");
  ...
  if (snapshot.value.reference === null) {
    ...
    return {
      ok: true,
      value: {
        snapshot: { ...snapshot.value, reference: null },
        reference: null,
      },
    };
  }
  const reference = parseImportedReferenceValue(
    value.reference,
    "invalid-request",
  );
  if (
    !reference.ok ||
    !sameMetadata(snapshot.value.reference, reference.value.metadata)
  )
    return failure("invalid-request");
  return {
    ok: true,
    value: {
      snapshot: { ...snapshot.value, reference: snapshot.value.reference },
      reference: reference.value,
    },
  };
}
...
export function parseSettingsPatch(
  value: unknown,
): Result<SettingsPatch, PublicError> {
  if (!isOwnDataRecord(value) || typeof value.kind !== "string")
    return failure("invalid-request");
  switch (value.kind) {
    case "visibility":
      return hasExactKeys(value, ["kind", "visible"]) &&
        typeof value.visible === "boolean"
        ? { ok: true, value: { kind: "visibility", visible: value.visible } }
        : failure("invalid-request");
    case "opacity":
      return hasExactKeys(value, ["kind", "opacity"]) &&
        typeof value.opacity === "number" &&
        Number.isFinite(value.opacity) &&
        value.opacity >= 0 &&
        value.opacity <= 1
        ...
        : failure("invalid-request");
    case "inversion":
      return hasExactKeys(value, ["kind", "inverted"]) &&
        typeof value.inverted === "boolean"
        ? { ok: true, value: { kind: "inversion", inverted: value.inverted } }
        : failure("invalid-request");
    case "sizing": {
      const sizing = parseSizingValue(value.sizing, "invalid-request");
      return hasExactKeys(value, ["kind", "sizing"]) && sizing.ok
        ? { ok: true, value: { kind: "sizing", sizing: sizing.value } }
        : failure("invalid-request");
    }
    case "interaction-mode":
      return hasExactKeys(value, ["kind", "interactionMode"]) &&
        isInteractionMode(value.interactionMode)
        ? {
            ok: true,
            value: {
    ...
    case "placement": {
      ...
      return hasExactKeys(value, ["kind", "placement"]) && placement.ok
        ? { ok: true, value: { kind: "placement", placement: placement.value } }
        : failure("invalid-request");
    }
    default:
      return failure("invalid-request");
...
export function parsePublicError(
  ...
  if (value.message !== PUBLIC_ERROR_MESSAGES[code])
    return failure("invalid-request");
...
export function parseTabState(value: unknown): Result<TabState, PublicError> {
  ...
  if (
    ...
    !snapshot.ok ||
    snapshot.value.origin !== origin.value ||
    deriveOrigin(url.value) !== origin.value ||
    !diagnostic.ok ||
    (diagnostic.value !== null &&
      diagnostic.value.error.code !== "image-render-failed")
  ) {
    return failure("invalid-request");
  }
  return {
...
function parseRenderDiagnostic(
  value: unknown,
): Result<RenderDiagnostic, PublicError> {
  if (!isOwnDataRecord(value) || !hasExactKeys(value, ["referenceId", "error"]))
    return failure("invalid-request");
  ...
  if (
    !referenceId.ok ||
    !error.ok ||
    error.value.code !== "image-render-failed"
  )
    return failure("invalid-request");
  return {
    ok: true,
    value: { referenceId: referenceId.value, error: error.value },
  };
}

export function parsePopupResponse<T>(
  value: unknown,
  parseValue: ValueParser<T>,
): Result<PopupResponse<T>, PublicError> {
  if (
    !isOwnDataRecord(value) ||
  ...
  if (value.ok) {
    ...
    if (!hasExactKeys(value, ["requestId", "ok", "value"]) || !parsedValue.ok)
      return failure("invalid-request");
    return {
      ok: true,
      value: { requestId: value.requestId, ok: true, value: parsedValue.value },
    };
  }
  const error = parsePublicError(value.error);
  if (!hasExactKeys(value, ["requestId", "ok", "error"]) || !error.ok)
    return failure("invalid-request");
  return {
    ok: true,
...
export function parsePopupRequest(
  value: unknown,
): Result<PopupRequest, PublicError> {
  if (
    !isOwnDataRecord(value) ||
    ...
    return failure("invalid-request");
  const url =
    value.url === undefined ? undefined : parseSupportedUrl(value.url);
  switch (value.kind) {
    case "get-tab-state":
      return hasExactKeys(value, ["kind", "requestId"])
        ? {
            ok: true,
            value: { kind: "get-tab-state", requestId: value.requestId },
          }
        : failure("invalid-request");
    case "register-site":
      return hasExactKeys(value, ["kind", "requestId", "url"]) && url?.ok
        ? {
            ok: true,
            value: {
              kind: "register-site",
  ...
  switch (value.kind) {
    case "hydrate-overlay": {
      ...
      return hasExactKeys(value, ["kind", "hydration"]) && hydration.ok
        ? {
            ok: true,
            value: { kind: "hydrate-overlay", hydration: hydration.value },
          }
        : failure("invalid-request");
    }
    case "apply-settings": {
      ...
      return hasExactKeys(value, ["kind", "snapshot"]) && snapshot.ok
        ? {
            ok: true,
            value: { kind: "apply-settings", snapshot: snapshot.value },
          }
        : failure("invalid-request");
    }
    case "clear-overlay": {
      const revision = readSafeInteger(value.revision);
      return hasExactKeys(value, ["kind", "revision"]) &&
        revision !== undefined &&
        revision >= 0
        ? { ok: true, value: { kind: "clear-overlay", revision } }
        : failure("invalid-request");
    }
    default:
      return failure("invalid-request");
...
export function parseContentEvent(
  value: unknown,
): Result<ContentEvent, PublicError> {
  if (!isOwnDataRecord(value) || typeof value.kind !== "string")
    return failure("invalid-request");
  const url =
    value.url === undefined ? undefined : parseSupportedUrl(value.url);
  switch (value.kind) {
    case "content-ready":
      return hasExactKeys(value, ["kind", "url"]) && url?.ok
        ? {
            ok: true,
            value: { kind: "content-ready", url: url.value.toString() },
          }
        : failure("invalid-request");
    case "placement-committed": {
      const placement = parsePlacement(value.placement);
      return hasExactKeys(value, ["kind", "url", "placement"]) &&
        url?.ok &&
        placement.ok
        ? {
            ok: true,
    ...
    case "image-load-failed": {
      const referenceId = parseReferenceId(value.referenceId);
      return hasExactKeys(value, ["kind", "url", "referenceId"]) &&
        url?.ok &&
        referenceId.ok
        ? {
            ok: true,
...
export function parseImageRecordV1(
  value: unknown,
): Result<ImageRecordV1, PublicError> {
  if (
    !isOwnDataRecord(value) ||
    !hasExactKeys(value, ["schemaVersion", "referenceId", "dataUrl"]) ||
...
export function parseOriginIndexV1(
  value: unknown,
): Result<OriginIndexV1, PublicError> {
  if (
    !isOwnDataRecord(value) ||
    !hasExactKeys(value, ["schemaVersion", "origins"]) ||
    value.schemaVersion !== 1 ||
  ...
  for (const originValue of value.origins) {
    ...
    if (!origin.ok || origins.includes(origin.value))
      return failure("invalid-stored-data");
