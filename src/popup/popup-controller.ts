...
import { parseOverlaySnapshot, parsePopupResponse, parseSupportedUrl, parseTabState } from "../shared/parse";
...
export type PopupState =
  ...
  | Readonly<{ kind: "enabled-reference"; tab: TabState; confirmingClear: boolean }>
  | Readonly<{
    kind: "error";
    error: PublicError;
...
export interface PopupImporter {
  (file: File): Promise<Readonly<{ ok: true; value: ImportedReference }> | Readonly<{ ok: false; error: PublicError }>>;
}

type StateWithTab = Extract<PopupState, { tab: TabState }>;
type RecoveryState = Extract<PopupState, { kind: "access-required" | "enabled-empty" | "enabled-reference" }>;
type PendingSettings = Readonly<{ patch: SettingsPatch; focusId: string }>;
type SnapshotRequest =
  | Readonly<{ kind: "register-site"; url: string }>
  | Readonly<{ kind: "replace-reference"; url: string; reference: ImportedReference }>
  | Readonly<{ kind: "update-settings"; url: string; patch: SettingsPatch }>;

const INVALID_RESPONSE: PublicError = { code: "invalid-request", message: "Pixel Pincher received an invalid request." };

function parseVoid(value: unknown): Readonly<{ ok: true; value: undefined }> | Readonly<{ ok: false; error: PublicError }> {
  return value === undefined ? { ok: true, value: undefined } : { ok: false, error: INVALID_RESPONSE };
}
...
export class PopupController {
  ...
  get state(): PopupState { return this.#state; }
  ...
  async enable(focusId = "enable-site"): Promise<void> {
    ...
    if (!granted) {
      this.#fail({ code: "site-access-denied", message: "Pixel Pincher needs permission for this site." }, recovery);
      return;
    }
    const response = await this.#sendSnapshot({ kind: "register-site", url: recovery.url }, focusId);
  ...
  async importFile(file: File, focusId = "reference-file"): Promise<void> {
    ...
    const response = await this.#sendSnapshot({ kind: "replace-reference", url: recovery.tab.url, reference: imported.value }, focusId);
    if (response !== undefined) this.#applySnapshot(response, focusId);
  }

  updateSettings(patch: SettingsPatch, focusId: string, coalesce = false): void {
    const recovery = this.#recover();
    if (!isEnabled(recovery) || recovery.tab.snapshot.reference === null) return;
    this.#clearError();
    ...
    if (this.#settingsInFlight) {
      ...
      if (coalesce) {
        const existing = this.#pendingSettings.findIndex((candidate) => candidate.patch.kind === patch.kind);
  ...
  setOpacity(percent: number, focusId = "opacity"): void {
    ...
    this.updateSettings({ kind: "opacity", opacity: bounded / 100 }, focusId, true);
  }
  ...
  resetScale(): void { this.setSizingPercent(100, "reset-scale"); }
  ...
  async clearSite(focusId = "clear-site"): Promise<void> {
    const state = this.#recover();
    const corruptDataClear = this.#state.kind === "error" && this.#state.error.code === "invalid-stored-data" && state.kind === "access-required";
  ...
  #requestId(): string { return `popup-${this.#nextRequest++}`; }

  async #sendTab(request: Omit<Extract<PopupRequest, { kind: "get-tab-state" }>, "requestId">, focusId: string): Promise<TabState | undefined> {
    const requestId = this.#requestId();
    try {
      const parsed = parsePopupResponse(await this.#adapter.send({ ...request, requestId }), parseTabState);
      if (!parsed.ok || parsed.value.requestId !== requestId) return this.#invalid(focusId);
      if (!parsed.value.ok) return this.#fail(parsed.value.error, this.#recover(), focusId);
      return parsed.value.value;
    } catch { return this.#invalid(focusId); }
  }

  async #sendSnapshot(request: SnapshotRequest, focusId: string): Promise<OverlaySnapshot | undefined> {
    const requestId = this.#requestId();
    try {
      const parsed = parsePopupResponse(await this.#adapter.send({ ...request, requestId }), parseOverlaySnapshot);
      if (!parsed.ok || parsed.value.requestId !== requestId) return this.#invalid(focusId);
      if (!parsed.value.ok) return this.#fail(parsed.value.error, this.#recover(), focusId);
      return parsed.value.value;
    } catch { return this.#invalid(focusId); }
  ...
  async #sendVoid(request: Omit<Extract<PopupRequest, { kind: "clear-site" }>, "requestId">, focusId: string): Promise<boolean> {
    const requestId = this.#requestId();
    try {
      const parsed = parsePopupResponse(await this.#adapter.send({ ...request, requestId }), parseVoid);
      if (!parsed.ok || parsed.value.requestId !== requestId) { this.#invalid(focusId); return false; }
      if (!parsed.value.ok) { this.#fail(parsed.value.error, this.#recover(), focusId); return false; }
  ...
  #applySnapshot(snapshot: OverlaySnapshot, focusId: string): void {
    ...
    if (prior !== undefined) this.#applyTab({ ...prior, enabled: true, snapshot, diagnostic: null });
      ...
      this.#applyTab({ tabId: 0, url: current.url, origin: snapshot.origin, enabled: true, snapshot, diagnostic: null });
    } else return;
    this.#view.restoreFocus(focusId);
  }
